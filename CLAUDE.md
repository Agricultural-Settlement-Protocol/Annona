...
- ✅ **WIRE-LAYER sync to v4.0 done (2026-07-12):** `packages/core` events (`AgreementCreatedData.supplier`+`subsidyTier`+`basePrice`, `SupplyDispatchedData.supplier`, `SettledData.principalToSupplier`, `RemittanceClearedData.supplier`) + 5 funding event interfaces + `SubsidyTier`/`FundingStatus`/`FundingRequest` types; the indexer decoder (`poll.ts` topic map incl. the 5 funding events) and reducer (`handlers.ts`) now read the v4.0 event field names. `pnpm check-types` green (7/7), reducer test 3/3. **Deliberate transitional split (bridged in the reducer):** the WIRE/event layer says `supplier`/`principalToSupplier`, while the **DB schema + read-model + 417 web display sites still say `agrinas`** — the reducer maps `d.supplier→agrinasId` col, `d.principalToSupplier→principal_to_agrinas` col. Funding + subsidy events are decoded and logged to `event_log` but NOT projected (no read-model tables yet), exactly like `Flagged`/`HarvestReceiptMinted`.
- ⬜ **DEFERRED — DB/read-model rename phase (separate):** rename the `agrinas` table/columns + `app_role` enum value + migration (live Supabase coordination — mind the 2026-07-11 duplicate-timestamp landmine), API read-model field names, the 417 web display sites, and add funding/subsidy read-model tables. **The two-role trap lives here:** `harvest_shipment.agrinas_id` = warehouse operator (NOT supplier); `agreement`/`saprotan_catalog`/`residu`/`coop` `.agrinas_id` = supplier — do NOT let a blind `sed` conflate them. Also `apps/web/lib/invocations.ts` (write path) is typecheck-green but STALE vs the new `create_agreement` signature (supplier/subsidy_tier) — belongs to the write-path/UI phase.
