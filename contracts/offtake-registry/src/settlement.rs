//! Pure settlement + flag-classification math (v4.0 three-way split).
//!
...
//! Then split debt_paid pro-rata against input_debt into principal (Supplier
//! residu) vs margin (KMP), assigning any integer-division dust to margin so
//! `residu_principal` never over-states Supplier's claim (Golden rule 6/6b).
...
pub struct Settlement {
    ...
    /// of debt_paid, Supplier's principal (residu owed back to Supplier)
    pub principal_to_supplier: i128,
    /// of debt_paid, KMP's markup margin (KMP keeps)
    pub coop_margin: i128,
}

/// Derive `input_debt` from the Supplier principal + KMP markup.
/// `input_debt = base_price * (10000 + saprotan_markup_bps) / 10000`.
...
/// current `remaining_debt`; `base_price` is the ORIGINAL principal. The
...
pub fn compute(
    ...
    // any rounding dust falls to margin so principal never over-states Supplier.
    let principal_to_supplier = if input_debt == 0 {
        0
    } else {
        debt_paid
    ...
    let coop_margin = debt_paid
        .checked_sub(principal_to_supplier)
        .ok_or(ContractError::MathOverflow)?;
