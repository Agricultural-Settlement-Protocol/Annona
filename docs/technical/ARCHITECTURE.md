...
>
> **v3.0 — multi-party (PMK 15/2026).** Three commercial parties (**Supplier** operator, **KMP** koperasi, **Farmer**) + a read-only **Government** regulator. Double-confirmation lifecycle, three-way split settlement, and residu reconciliation. See §0.
...
```
  ...
  ┌───────────────────┐        ┌──────────────────────────────────────────┐
  │ GOVERNMENT        │        │ SUPPLIER (operator) ↔ KMP (koperasi) ↔     │
...
- **Supplier** owns the master saprotan catalog (`base_price_supplier` = principal), dispatches logistics, and verifies residu remittance. Operator, not regulator.
...
**Dashboards (3 shells, not 4):** KMP dashboard · Oversight dashboard (RBAC → Supplier operator view + Government regulator view) · Farmer view. Supplier is a role inside the oversight app, not a separate product.
...
3. **App on top, protocol underneath.** The UI is KMP/Supplier-specific. The contracts are commodity-agnostic so coffee/fish/cacao coops — and other developers — can build on the same primitives.
...
5. **Split allocation is on-chain, remittance is off-chain-verified.** The residu split (whose money is whose) is locked on-chain; the actual rupiah bank transfer of Supplier's principal is verified off-chain then anchored (`confirm_remittance`). This is the anti-moral-hazard guarantee.
...
2. CREATE AGREEMENT (draft = collective Surat Pesanan)
   web(KMP) ─build tx─► Freighter sign ─► offtake-registry.create_agreement()
        · picks catalog item → base_price_supplier (Supplier, read-only)
...
gross = volume × HPP
  ...
  │     ├─ residu_principal (base_price_supplier)     → Supplier (remit back)
  │     └─ coop_margin      (markup portion)         → KMP keeps
  └─ net_to_farmer = (gross − handling_cut) − input_debt → farmer (dIDR / rupiah)
```

Only `net_to_farmer` moves as money in the demo (dIDR transfer). `residu_principal` and KMP's cuts are **on-chain accruals** mirroring the rupiah that physically stays in KMP's cash box until the principal is remitted to Supplier and confirmed on-chain (`confirm_remittance`). Worked numbers: `SMART-CONTRACT.md` §5.
...
|---|---|
| Agreement id, farmer/coop/supplier addresses, commodity code/grade/moisture/hpp-version | Farmer name, KTP raw, phone, GPS |
| base_price_supplier, saprotan_markup_bps, input_debt (derived), hpp_handling_fee_bps | Saprotan catalog, input-basket line items |
...
- **Auth:** each write is bound to its party — `coop.require_auth()` (KMP: create/accept/deliver/settle/remit), `supplier.require_auth()` (dispatch/confirm/dispute), `admin.require_auth()` (resolve). `settle` caller auth (KMP in MVP; service/multisig in prod). The two confirmation gates (Supplier dispatch, KMP accept) mean no single party can advance the other's step. Full multisig role separation = roadmap. Detail in `./SMART-CONTRACT.md` §9.
