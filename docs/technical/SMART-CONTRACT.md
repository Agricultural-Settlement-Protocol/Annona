...
impl OfftakeRegistry {
    ...
    /// amount_reconciled (CLAMPED at amount_disbursed); Disbursed -> Reconciled once fully repaid.
    /// Auth mirrors settle (§9): the request's `coop` OR the admin/service key — so the Path A
    /// backend that drives settle can drive this repayment leg too. (As-built resolution of the
    /// §4-vs-§9 wording: coop-or-admin wins, per §9's auth table + the settle automation path.)
    /// Open Decision B: kept a SEPARATE explicit call in MVP; auto-hook inside settle() is a v4.1 nicety.
    pub fn reconcile_funding(env: Env, caller: Address, id: u64, principal_collected: i128);
...
- **Unit tests** (`soroban_sdk::testutils`): **v4.0 built, ✅ 54 tests green** in `contracts/offtake-registry/src/test.rs` (`cd contracts && cargo test`). Covers: constructor, both settlement worked examples (single + staged), every flag band, flag healing, force-majeure, debt-exceeds-gross floor-at-0, `NothingToSettle`, auth binding (wrong coop / stranger / admin-allowed), TTL extension, i128 overflow guard; the full double-confirmation lifecycle incl. `SupplyDispatched`/`Active` gates (cannot `accept_supply` before `dispatch_supply`, cannot `record_delivery` before `Active`); derived `input_debt` math; three-way split incl. pro-rata principal/margin + rounding dust to margin; residu lifecycle (Pending→Remitted→Cleared) + dispute freeze/unfreeze/resolve; supplier auth binding on dispatch/confirm/dispute. **v4.0 adds (all green):** subsidy tier recorded verbatim (`Subsidized`/`Commercial`; the `Subsidized`==HET invariant is off-chain, the contract trusts the submitted tier); the funding lifecycle (`Requested→Approved→Disbursed→Reconciled`, reject path, approve-above-request rejected, disburse-before-approve rejected, reconcile-before-disburse rejected, reconcile caps at disbursed) + wrong-financier/wrong-coop auth binding + `FundingNotFound`. **One test earns special note:** `disburse_funding` moves dIDR *from the financier* (not the contract balance), so the real Freighter flow needs the financier to authorize the nested `token.transfer` sub-invocation — `mock_all_auths` would hide a broken contract, so a dedicated test asserts via `env.auths()` that the sub-invocation auth is actually recorded.
...
- **Deploy:** `scripts/fund-testnet.sh` (identities) → `scripts/deploy.sh` = `stellar contract build` → deploy dIDR SAC → `stellar contract deploy` registry with `__constructor(admin, didr_sac)` → pre-fund → writes `scripts/artifacts.testnet.json` (contract ids + WASM hash). **v4.0 WASM = ~46 KB (limit 64 KB, ~18 KB headroom)** as built (`target/wasm32v1-none/release/offtake_registry.wasm`) — the funding enum + `FundingRequest` struct + 6 fns + subsidy tier added ~6 KB over the v3.0 ~40 KB. Comfortable; if a future addition tightens the budget, the `reject_funding` path or a `CoopFunding` index is the first cut (keep the four happy-path funding fns).
