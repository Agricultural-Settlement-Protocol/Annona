...
- **Grounding:** read-only, fed only indexer read-models; every number links to a source agreement/event (no hallucinated figures). **Role-scoped** — the Supplier view queries residu/dispatch/commercial models; the Government view queries macro production/reputation/flags. Swappable for Claude Haiku via a provider interface.
